...
app.post('/api/acta/guardar', (req, res) => {
    ...
    // 3. Guardar en acta_partido inyectando los puntos totales calculados en la columna 'puntos'
       // CORRECCIÓN: Añadida j.video_ruta a la consulta de la base de datos
    const queryActa = `
        SELECT a.id_jugador, a.id_acta, a.rol, a.posicion_x, a.posicion_y, 
               j.id_equipo, j.nombre, j.posicion, j.foto_ruta, j.video_ruta 
        FROM acta_partido a
        JOIN jugadores j ON a.id_jugador = j.id_jugador
        WHERE a.id_partido = ?`;
...
// 9. ALGORITMO CORREGIDO: SE INDEPENDIZA Y ASEGURA EL ENVÍO DE 'video_ruta' A LA FICHA
app.get('/api/jugador-detalle/:id_jugador', (req, res) => {
    ...
    // 2. Consulta para extraer todos los partidos que ha jugado, incluyendo la foto y el vídeo maestro del club
    ...
    db.query(queryRank, [id_j], (err, rankRes) => {
        ...
        db.query(queryHistorial, [id_j], (err, partidos) => {
            ...
            // Extraemos los datos multimedia del primer registro del jugador encontrado
            ...
            res.json({
                ...
                video_ruta: vRuta, // ⬅️ Enviamos la ruta web del vídeo de forma independiente
                foto_ruta: fRuta,
