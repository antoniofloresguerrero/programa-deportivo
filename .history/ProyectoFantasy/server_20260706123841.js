...
// =======================================================================
// PASARELA DEFINITIVA REPARADA: CONECTA LOS PARTIDOS REALES DE TU EQUIPO
// =======================================================================
app.get('/api/partidos', (req, res) => {
    // Capturamos el ID del equipo enviado por el frontend (?id_equipo=1)
    const idEquipoFiltro = req.query.id_equipo;
    
    console.log("📅 Ejecutando consulta de calendario real para el Club ID: " + idEquipoFiltro);

    let queryHistorialPartidos = "";
    let parametrosSQL = [];

    // SI EL FRONTEND ENVÍA UN ID DE EQUIPO, FILTRAMOS DE FORMA INTELIGENTE LOCAL Y VISITANTE
    if (idEquipoFiltro) {
        queryHistorialPartidos = `
            SELECT 
                p.*,
                COALESCE(eq_l.nombre, 'Club Local') AS equipo_local,
                COALESCE(eq_v.nombre, 'Club Visitante') AS equipo_visitante
            FROM partidos p
            LEFT JOIN equipos eq_l ON p.id_local = eq_l.id_equipo
            LEFT JOIN equipos eq_v ON p.id_visitante = eq_v.id_equipo
            WHERE p.id_local = ? OR p.id_visitante = ?
            ORDER BY p.jornada ASC, p.id_partido ASC`;
        parametrosSQL = [idEquipoFiltro, idEquipoFiltro];
    } else {
        // Fallback: Si no hay filtro, devuelve el calendario general del torneo
        queryHistorialPartidos = `
            SELECT 
                p.*,
                COALESCE(eq_l.nombre, 'Club Local') AS equipo_local,
    ...
    db.query(queryHistorialPartidos, parametrosSQL, (err, rows) => {
        if (err) {
            console.error("🔴 Error crítico de sintaxis en la consulta de partidos de MySQL:", err);
            return res.status(500).json({ error: err.message });
        }
        // Enviamos las filas reales directas de tu base de datos al navegador
        res.json(rows || []);
    });
});
