...
// 15. API DE SINCRONIZACIÓN DEFINITIVA: Resetea la BD antes de calcular para eliminar partidos borrados
app.post('/api/clasificacion/sincronizar', (req, res) => {
    console.log("Iniciando purga de puntos antiguos y recalculo real de LaLiga...");

    // PASO 1: Comando de limpieza absoluta. Ponemos a 0 todos los equipos para borrar residuos de partidos eliminados
    const queryResetFisico = `
        UPDATE equipos 
        SET pt = 0, pj = 0, pg = 0, pe = 0, pp = 0, gf = 0, gc = 0`;

    db.query(queryResetFisico, (errReset) => {
        if (errReset) {
            console.error("Error al resetear la tabla equipos:", errReset);
        ...
        // PASO 2: Extraemos únicamente los partidos reales de tu captura que tengan marcadores asignados
        ...
        db.query(queryPartidosActivos, (errPartidos, partidos) => {
            if (errPartidos) return res.status(500).json({ success: false, error: errPartidos.message });

            // Si la tabla partidos está vacía, terminamos aquí dejando todo a cero de forma limpia
            if (partidos.length === 0) {
                return res.json({ success: true, mensaje: "Clasificación reseteada a cero. No hay partidos disputados." });
            }

            // Mapa en memoria para ir sumando los goles y puntos de los partidos 13, 14 y 15
            let stats = {};

            // Inicializamos el mapa con todos los clubes para asegurar que ninguno falle
            db.query('SELECT id_equipo FROM equipos', (errEq, clubes) => {
                ...
                // Procesamos los resultados de los enfrentamientos reales activos
                partidos.forEach(p => {
                    let idL = p.id_local;
                ...
                // PASO 3: Inyectamos los nuevos datos relacionales puros mediante promesas UPDATE
                ...
                Promise.all(promesasUpdate)
                    .then(() => {
                        console.log("💾 ¡Clasificación real guardada con éxito libre de residuos antiguos!");
