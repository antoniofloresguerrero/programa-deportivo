...
// =======================================================================
// PASARELA HISTORIAL ADAPTABLE: ENLAZA LOS NOMBRES REALES DE TU WORKBENCH
// =======================================================================
app.get('/api/partidos', (req, res) => {
    console.log("📅 Procesando consulta inteligente de partidos en MySQL Workbench...");
    
    // CONSULTA TOLERANTE: Intentamos enlazar por la clave primaria estándar 'id_equipo'.
    // Si tus tablas de MySQL usan otra nomenclatura, el motor capturará los datos de respaldo.
    const queryHistorialFlexible = `
        SELECT 
            p.*,
            COALESCE(el.nombre, 'Equipo Local') AS equipo_local,
            COALESCE(ev.nombre, 'Equipo Visitante') AS equipo_visitante
        FROM partidos p
        LEFT JOIN equipos el ON (p.id_local = el.id_equipo OR p.id_equipo_local = el.id_equipo OR p.local_id = el.id_equipo)
        LEFT JOIN equipos ev ON (p.id_visitante = ev.id_equipo OR p.id_equipo_visitante = ev.id_equipo OR p.visitante_id = ev.id_equipo)
        ORDER BY p.jornada ASC`;

    db.query(queryHistorialFlexible, (err, rows) => {
        if (err) {
            console.warn("🔄 Plan B: Reintentando consulta con estructura de ID corta (id)...");
            
            // SEGUNDA PASARELA DE RESPALDO: Por si tu tabla equipos usa la clave primaria 'id' en lugar de 'id_equipo'
            const queryHistorialIdCorta = `
                SELECT 
                    p.*,
                    COALESCE(el.nombre, 'Equipo Local') AS equipo_local,
                    COALESCE(ev.nombre, 'Equipo Visitante') AS equipo_visitante
                FROM partidos p
                LEFT JOIN equipos el ON (p.id_local = el.id OR p.id_equipo_local = el.id)
                LEFT JOIN equipos ev ON (p.id_visitante = ev.id OR p.id_equipo_visitante = ev.id)
                ORDER BY p.jornada ASC`;

            return db.query(queryHistorialIdCorta, (errCorta, rowsCorta) => {
                if (errCorta) {
                    console.error("🔴 Error definitivo de lectura en las columnas de la tabla partidos:", errCorta);
                    return res.status(500).json({ error: errCorta.message });
                }
                res.json(rowsCorta || []);
            });
        }
