        ...
        db.query(queryHistorialGlobal, (errHist, todasLasNotas) => {
            ...
            todasLasNotas.forEach(nota => {
                ...
                mapaHistorial[nota.id_jugador].push(parseInt(nota.puntos) || 0);
                       ...
                       // C. ALGORITMO CORREGIDO: Divide estrictamente por la cantidad real de partidos disputados (MÁXIMO 3)
            let actaCalculadaConMedia = jugadoresActa.map(j => {
                let notasPasadas = mapaHistorial[j.id_jugador] || [];

                // Recogemos únicamente los últimos 3 partidos jugados de su historial real
                ...
                if (cantidadPartidosReales > 0) {
                    // Sumamos las notas reales de sus encuentros disputados
                    let sumaPuntos = ultimas3Notas.reduce((a, b) => a + b, 0);
                    
                    // REGLA DE RESCATE: Si su nota en los últimos 3 partidos es 0 absoluto, se le asigna 5
                    if (sumaPuntos === 0 && cantidadPartidosReales === 3) {
                        mediaCalculada = 5.0;
                    } else {
                        // CORRECCIÓN CLAVE: Dividimos de forma exacta entre la cantidad real de partidos (1, 2 o 3)
                        // Usamos parseFloat para mantener los decimales nítidos como el 10.5 de tu captura
                        mediaCalculada = parseFloat((sumaPuntos / cantidadPartidosReales).toFixed(1));
                    }
                } else {
                    // Si es el partido inicial absoluto de la liga, arranca con su nota base
                    mediaCalculada = parseFloat((parseInt(j.puntos) || 0).toFixed(1));
                }

                // Inyectamos el promedio móvil exacto con decimales en las variables de la interfaz
