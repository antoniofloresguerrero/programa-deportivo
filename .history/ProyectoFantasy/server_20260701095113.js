...
// =======================================================================
// 15. API DE SINCRONIZACIÓN DEFINITIVA CON DESBLOQUEO DE SEGURIDAD MYSQL
// =======================================================================
app.post('/api/clasificacion/sincronizar', (req, res) => {
    console.log("Iniciando purga de puntos antiguos y recalculo real de LaLiga...");

    // PASO 1: Vaciado físico usando un filtro compatible con Safe Updates (id_equipo > 0)
    // Esto fulmina los puntos fantasma de partidos eliminados de forma inmediata
    const queryResetSeguro = `
        UPDATE equipos 
        SET pt = 0, pj = 0, pg = 0, pe = 0, pp = 0, gf = 0, gc = 0 
        WHERE id_equipo > 0`;

    db.query(queryResetSeguro, (errReset) => {
        if (errReset) {
            console.error("Error crítico al resetear la tabla equipos:", errReset);
            return res.status(500).json({ success: false, error: errReset.message });
        }

        console.log("🧼 Tabla equipos purgada con éxito. Leyendo partidos reales activos...");

        // PASO 2: Extraemos únicamente los partidos reales vigentes que tengan marcadores asignados
        ...
        db.query(queryPartidosActivos, (errPartidos, partidos) => {
            ...
            // Si la tabla partidos estuviese vacía, cerramos aquí dejando todo a cero de forma limpia
            if (partidos.length === 0) {
                return res.json({ success: true, mensaje: "Clasificación reseteada a cero. No hay partidos disputados." });
            }

            // Estructura temporal en memoria para acumular las estadísticas de los partidos reales
            let stats = {};

            // Traemos todos los clubes existentes para inicializar el mapa posicional
            db.query('SELECT id_equipo FROM equipos', (errEq, clubes) => {
                ...
                // Procesamos los resultados reales de tus enfrentamientos activos (13, 14, 15...)
                partidos.forEach(p => {
                    let idL = p.id_local;
                ...
                // PASO 3: Inyectamos los nuevos datos consolidados puros en tu base de datos
                ...
                Promise.all(promesasUpdate)
                    .then(() => {
                        console.log("💾 ¡Base de datos de LaLiga actualizada físicamente libre de residuos!");
                        res.json({ success: true, mensaje: "Clasificación oficial sincronizada correctamente de forma limpia." });
                    })
                    .catch(errPromise => {
                        console.error("Error al inyectar actualización limpia:", errPromise);
