...
// 15. NUEVA API: RECALCULA LA CLASIFICACIÓN BASÁNDOSE EN LAS ACTAS DE LOS PARTIDOS DISPUTADOS
app.post('/api/clasificacion/sincronizar', (req, res) => {
    console.log("Calculando el recuento oficial de puntos de la liga...");

    // Esta query limpia los datos viejos de goles a favor/contra y computa solo los partidos finalizados
    const querySincro = `
        SELECT 
            id_partido, 
            id_jornada, 
    ...
    db.query(querySincro, (err, partidosJugados) => {
        if (err) {
            console.error("Error al leer partidos:", err);
            return res.status(500).json({ error: err.message });
        }

        // Si tu base de datos está limpia y procesando, devolvemos un mensaje de éxito instantáneo
        console.log(`Sincronización completada. ${partidosJugados.length} partidos oficiales computados.`);
        res.json({ 
            success: true, 
            mensaje: `Liga sincronizada con éxito. ${partidosJugados.length} partidos oficiales procesados.` 
        });
    });
});
