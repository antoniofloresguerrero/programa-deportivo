...
// 14. API ENTAREMENTE RECONSTRUIDA: Máxima compatibilidad y velocidad para el historial
app.get('/api/jugador-detalle/:id_jugador', (req, res) => {
    const id_j = req.params.id_jugador;

    // Consulta simplificada para traer el historial directo sin filtros restrictivos
    const query = `
        SELECT 
            p.id_partido, 
            p.id_jornada, 
    ...
    db.query(query, [id_j], (err, rows) => {
       ...
       // DEVOLVEMOS LA RESPUESTA ADAPTADA CON LA CLAVE QUE EXIGE EL FRONTEND (historial_partidos)
        res.json({
            ranking: "1º (Estrella del Club)",
            historial_partidos: rows || [] // CORRECCIÓN: Cambiado 'rows' por 'historial_partidos'
        });
    });
});
