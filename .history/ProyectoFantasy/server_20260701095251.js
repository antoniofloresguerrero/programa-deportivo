...
// =======================================================================
// 15. API DE SINCRONIZACIÓN MAESTRA COMPATIBLE CON CUALQUIER NOMBRE DE COLUMNA
// =======================================================================
app.post('/api/clasificacion/sincronizar', (req, res) => {
    console.log("Iniciando recalculo dinámico libre de errores de columna...");

    // 1. Extraemos los partidos jugados reales vigentes de tu captura
    const queryPartidosActivos = `
        SELECT id_local, id_visitante, goles_local, goles_visitante 
        FROM partidos 
        WHERE goles_local IS NOT NULL AND goles_visitante IS NOT NULL`;

    db.query(queryPartidosActivos, (errPartidos, partidos) => {
        if (errPartidos) {
            console.error("Error al leer partidos activos:", errPartidos);
            return res.status(500).json({ success: false, error: errPartidos.message });
        }

        // Estructura temporal en memoria para calcular los puntos reales
        let stats = {};

        // 2. Escaneamos la tabla equipos para ver cómo se llaman tus columnas reales
        db.query('DESCRIBE equipos', (errDesc, columnas) => {
            if (errDesc) return res.status(500).json({ success: false, error: errDesc.message });

            // Detectamos de forma automática si usas nombres cortos (pt, pj) o largos (puntos, partidos_jugados)
            let colNombres = {
                pt: columnas.find(c => c.Field === 'pt' || c.Field === 'puntos' || c.Field === 'puntos_totales')?.Field || 'pt',
                pj: columnas.find(c => c.Field === 'pj' || c.Field === 'partidos_jugados' || c.Field === 'pj_total')?.Field || 'pj',
                pg: columnas.find(c => c.Field === 'pg' || c.Field === 'partidos_ganados' || c.Field === 'ganados')?.Field || 'pg',
                pe: columnas.find(c => c.Field === 'pe' || c.Field === 'partidos_empatados' || c.Field === 'empatados')?.Field || 'pe',
                pp: columnas.find(c => c.Field === 'pp' || c.Field === 'partidos_perdidos' || c.Field === 'perdidos')?.Field || 'pp',
                gf: columnas.find(c => c.Field === 'gf' || c.Field === 'goles_favor' || c.Field === 'goles_marcados')?.Field || 'gf',
                gc: columnas.find(c => c.Field === 'gc' || c.Field === 'goles_contra' || c.Field === 'goles_encajados')?.Field || 'gc'
            };

            console.log("🔍 Columnas detectadas en tu MySQL Workbench:", colNombres);

            // 3. Primero ponemos a cero físico todas las columnas detectadas de forma segura
            const queryResetSeguro = `
                UPDATE equipos 
                SET ${colNombres.pt} = 0, ${colNombres.pj} = 0, ${colNombres.pg} = 0, ${colNombres.pe} = 0, ${colNombres.pp} = 0, ${colNombres.gf} = 0, ${colNombres.gc} = 0 
                WHERE id_equipo > 0`;

            db.query(queryResetSeguro, (errReset) => {
                if (errReset) return res.status(500).json({ success: false, error: errReset.message });

                // Inicializamos el mapa en memoria con los IDs reales
                db.query('SELECT id_equipo FROM equipos', (errEq, clubes) => {
                    if (errEq) return res.status(500).json({ success: false, error: errEq.message });

                    clubes.forEach(c => {
                        stats[c.id_equipo] = { pt: 0, pj: 0, pg: 0, pe: 0, pp: 0, gf: 0, gc: 0 };
                    });

                    // Procesamos los marcadores reales de tus enfrentamientos activos (13, 14, 15)
                    partidos.forEach(p => {
                        let idL = p.id_local; let idV = p.id_visitante;
                        let gL = parseInt(p.goles_local) || 0; let gV = parseInt(p.goles_visitante) || 0;
                        ...
                        if (stats[idL] && stats[idV]) {
                            ...
                            if (gL > gV) { stats[idL].pt += 3; stats[idL].pg++; stats[idV].pp++; } 
                            else if (gV > gL) { stats[idV].pt += 3; stats[idV].pg++; stats[idL].pp++; } 
                            else { stats[idL].pt += 1; stats[idV].pt += 1; stats[idL].pe++; stats[idV].pe++; }
                        }
                    });

                    // 4. Inyectamos la actualización usando los nombres de columna reales que descubrió el escaner
                    let promesasUpdate = Object.keys(stats).map(id_club => {
                        let s = stats[id_club];
                        const queryUpdateDinamico = `
                            UPDATE equipos 
                            SET ${colNombres.pt} = ?, ${colNombres.pj} = ?, ${colNombres.pg} = ?, ${colNombres.pe} = ?, ${colNombres.pp} = ?, ${colNombres.gf} = ?, ${colNombres.gc} = ? 
                            WHERE id_equipo = ?`;
                        
                        return new Promise((resolve, reject) => {
                            db.query(queryUpdateDinamico, [s.pt, s.pj, s.pg, s.pe, s.pp, s.gf, s.gc, id_club], (errUp) => {
                                if (errUp) reject(errUp); else resolve();
                            });
                        });
                    });
                    ...
                    Promise.all(promesasUpdate)
                        .then(() => {
                            console.log("💾 Sincronización finalizada correctamente sin residuos viejos.");
                            res.json({ success: true, mensaje: "Clasificación oficial sincronizada correctamente." });
                        })
                        .catch(errPromise => {
                            console.error("Error al inyectar actualización dinámica:", errPromise);
