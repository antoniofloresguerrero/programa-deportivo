...
// =======================================================================
// 5. API DEL ACTA CON ALGORITMO FANTASY DE MEDIA MÓVIL AUTOMÁTICA (3 PARTIDOS)
// =======================================================================
app.get('/api/acta/:id_partido', (req, res) => {
    const id_p = req.params.id_partido;

    // A. Traemos los jugadores que pertenecen a este acta de partido activa
    const queryActaBase = `
        SELECT ap.*, j.nombre, j.posicion, j.foto_ruta, j.dorsal
    ...
    db.query(queryActaBase, [id_p], (err, jugadoresActa) => {
        ...
        // B. QUERY DE MEMORIA: Buscamos el historial completo de notas pasadas de estos futbolistas
        ...
        db.query(queryHistorialGlobal, (errHist, todasLasNotas) => {
            ...
            // Agrupamos el historial por cada ID de jugador
            let mapaHistorial = {};
            ...
            // C. ALGORITMO: Calculamos la media móvil exacta para cada jugador antes de mandarlo a la web
            let actaCalculadaConMedia = jugadoresActa.map(j => {
                let notasPasadas = mapaHistorial[j.id_jugador] || [];

                // Solo nos interesan los últimos 3 partidos jugados de su historial
                let ultimas3Notas = notasPasadas.slice(0, 3);
                let cantidadPartidos = ultimas3Notas.length;

                let mediaCalculada = 0;

                if (cantidadPartidos > 0) {
                    // Sumamos las notas de sus últimos encuentros
                    let sumaPuntos = ultimas3Notas.reduce((a, b) => a + b, 0);
                    
                    // REGLA DE RESCATE CRUCIAL SOLICITADA: Si su nota en los últimos 3 partidos es 0, le asignamos 5
                    if (sumaPuntos === 0 && cantidadPartidos === 3) {
                        mediaCalculada = 5;
                    } else {
                        // Media móvil tradicional según los partidos disputados hasta el momento (1, 2 o 3)
                        mediaCalculada = Math.round(sumaPuntos / cantidadPartidos);
                    }
                } else {
                    // Si es el partido 1 y no tiene historial, arranca limpio en 0 o la nota base asignada
                    mediaCalculada = parseInt(j.puntos) || 0;
                }

                // Inyectamos la media automática tanto en 'puntos' como en 'valor_actual' para la web
            ...
            // Devolvemos la nómina de jugadores con sus promedios ya inyectados
