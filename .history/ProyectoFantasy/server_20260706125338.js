...
// =======================================================================
// PASARELA HISTORIAL REPARADA: CONSULTA DIRECTA A LA TABLA PARTIDOS
// =======================================================================
app.get('/api/partidos', (req, res) => {
    const idEquipoFiltro = req.query.id_equipo;
    console.log("📅 Solicitando historial de partidos de MySQL para el Club ID: " + idEquipoFiltro);

    let queryHistorial = "SELECT * FROM partidos";
    let parametros = [];

    // Si el frontend envía el parámetro ?id_equipo=1, filtramos de forma directa
    if (idEquipoFiltro) {
        queryHistorial = "SELECT * FROM partidos WHERE id_local = ? OR id_visitante = ? ORDER BY jornada ASC";
        parametros = [idEquipoFiltro, idEquipoFiltro];
    } else {
        queryHistorial = "SELECT * FROM partidos ORDER BY jornada ASC";
    }

    db.query(queryHistorial, parametros, (err, rows) => {
        if (err) {
            console.error("🔴 Error de lectura en la tabla partidos de MySQL:", err);
            return res.status(500).json({ error: err.message });
        }
        // Enviamos las filas reales y crudas de tu base de datos hacia el frontend
        res.json(rows || []);
    });
});
