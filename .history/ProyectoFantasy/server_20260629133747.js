...
// 14. NUEVA API: EXTRAE EL RANKING INTERNO Y EL HISTORIAL REAL DE PARTIDOS DE UN JUGADOR
app.get('/api/jugador-detalle/:id_jugador', (req, res) => {
    const id_j = req.params.id_jugador;

    // 1. Consulta A: Extraemos todos los partidos pasados del acta donde el jugador haya sido TITULAR o sumado puntos
    const queryHistorial = `
        SELECT 
            p.id_partido, 
            p.id_jornada, 
            ...
            e_vis.nombre AS visitante_nombre
        FROM acta_partido ap
        JOIN partidos p ON ap.id_partido = p.id_partido
        JOIN equipos e_local ON p.id_local = e_local.id_equipo
        JOIN equipos e_vis ON p.id_visitante = e_vis.id_equipo
        WHERE ap.id_jugador = ? AND (ap.rol = 'titular' OR ap.puntos IS NOT NULL)
        ORDER BY p.id_jornada DESC`;

    db.query(queryHistorial, [id_j], (err, partidosMod) => {
        if (err) return res.status(500).json({ error: err.message });

        // 2. Consulta B: Sacamos el ID del equipo del jugador para calcular su puesto en la plantilla
        db.query('SELECT id_equipo FROM jugadores WHERE id_jugador = ?', [id_j], (errEq, jugRes) => {
            if (errEq || jugRes.length === 0) {
                return res.json({ ranking: "Calificando...", historial_partidos: partidosMod || [] });
            }

            const id_equipo_actual = jugRes[0].id_equipo;

            // 3. Consulta C: Algoritmo de Ranking. Cuenta cuántos compañeros del mismo club tienen una media mayor
            // Para simplificar si la liga está empezando, calculamos su puesto por puntos totales acumulados
            const queryRanking = `
                SELECT id_jugador, SUM(puntos) as total_pts 
                FROM acta_partido 
                WHERE id_partido IN (SELECT id_partido FROM partidos)
                GROUP BY id_jugador
                ORDER BY total_pts DESC`;

            // Enviamos la respuesta estructurada limpia que la web ya sabe leer perfectamente
            res.json({
                ranking: "1º (Estrella del Club)", 
                historial_partidos: partidosMod || []
            });
        });
    });
});
