...
// =======================================================================
// PASARELA HISTORIAL: TRAE TODOS LOS PARTIDOS CON SUS NOMBRES DE EQUIPOS
// =======================================================================
app.get('/api/partidos', (req, res) => {
    console.log("📅 Recibiendo solicitud masiva de partidos para el historial de LaLiga");
    
    // Consulta relacional limpia que enlaza las claves de los clubes con sus nombres reales
    const queryHistorialPartidos = `
        SELECT 
            p.id_partido,
            p.jornada,
            p.id_local,
    ...
    db.query(queryHistorialPartidos, (err, rows) => {
        if (err) {
            console.error("🔴 Error al leer la tabla partidos en MySQL:", err);
            return res.status(500).json({ error: err.message });
        }
        // Devolvemos el array puro y limpio hacia tu navegador
        res.json(rows || []);
    });
});
