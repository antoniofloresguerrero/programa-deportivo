...
// =======================================================================
// 11. API DE CLASIFICACIÓN RECONSTRUIDA: CÁLCULO EN VIVO RECOGIENDO EXCLUSIVAMENTE PARTIDOS ACTIVOS
// =======================================================================
app.get('/api/clasificacion', (req, res) => {
    console.log("Calculando clasificación matemática en vivo basada únicamente en partidos reales...");

    // Esta superconsulta en vivo genera los totales, partidos en casa y fuera partiendo de cero absoluto
    const queryMaestraEnVivo = `
        SELECT 
            e.id_equipo,
            e.nombre,
            
            -- 1. TOTALES GENERALES
            COALESCE(SUM(CASE WHEN p.goles_local IS NOT NULL AND p.goles_visitante IS NOT NULL THEN 1 ELSE 0 END), 0) as pj,
            COALESCE(SUM(
                CASE 
            ...
            COALESCE(SUM(CASE WHEN (p.id_local = e.id_equipo AND p.goles_local > p.goles_visitante) OR (p.id_visitante = e.id_equipo AND p.goles_visitante > p.goles_local) THEN 1 ELSE 0 END), 0) as pg,
            COALESCE(SUM(CASE WHEN p.goles_local = p.goles_visitante AND p.goles_local IS NOT NULL THEN 1 ELSE 0 END), 0) as pe,
            COALESCE(SUM(CASE WHEN (p.id_local = e.id_equipo AND p.goles_local < p.goles_visitante) OR (p.id_visitante = e.id_equipo AND p.goles_visitante < p.goles_local) THEN 1 ELSE 0 END), 0) as pp,
            COALESCE(SUM(CASE WHEN p.id_local = e.id_equipo THEN p.goles_local WHEN p.id_visitante = e.id_equipo THEN p.goles_visitante ELSE 0 END), 0) as gf,
            COALESCE(SUM(CASE WHEN p.id_local = e.id_equipo THEN p.goles_visitante WHEN p.id_visitante = e.id_equipo THEN p.goles_local ELSE 0 END), 0) as gc,

            -- 2. ESTADÍSTICAS EN CASA (Mapeado exacto a tu diseño extendido)
            COALESCE(SUM(CASE WHEN p.id_local = e.id_equipo AND p.goles_local IS NOT NULL THEN 1 ELSE 0 END), 0) as pj_casa,
            COALESCE(SUM(CASE WHEN p.id_local = e.id_equipo AND p.goles_local > p.goles_visitante THEN 3 WHEN p.id_local = e.id_equipo AND p.goles_local = p.goles_visitante THEN 1 ELSE 0 END), 0) as pt_casa,
            COALESCE(SUM(CASE WHEN p.id_local = e.id_equipo AND p.goles_local > p.goles_visitante THEN 1 ELSE 0 END), 0) as pg_casa,
            COALESCE(SUM(CASE WHEN p.id_local = e.id_equipo AND p.goles_local = p.goles_visitante THEN 1 ELSE 0 END), 0) as pe_casa,
            COALESCE(SUM(CASE WHEN p.id_local = e.id_equipo AND p.goles_local < p.goles_visitante THEN 1 ELSE 0 END), 0) as pp_casa,
            COALESCE(SUM(CASE WHEN p.id_local = e.id_equipo THEN p.goles_local ELSE 0 END), 0) as gf_casa,
            COALESCE(SUM(CASE WHEN p.id_local = e.id_equipo THEN p.goles_visitante ELSE 0 END), 0) as gc_casa,

            -- 3. ESTADÍSTICAS FUERA DE CASA (Mapeado exacto a tu diseño extendido)
            COALESCE(SUM(CASE WHEN p.id_visitante = e.id_equipo AND p.goles_visitante IS NOT NULL THEN 1 ELSE 0 END), 0) as pj_fuera,
            COALESCE(SUM(CASE WHEN p.id_visitante = e.id_equipo AND p.goles_visitante > p.goles_local THEN 3 WHEN p.id_visitante = e.id_equipo AND p.goles_local = p.goles_visitante THEN 1 ELSE 0 END), 0) as pt_fuera,
            COALESCE(SUM(CASE WHEN p.id_visitante = e.id_equipo AND p.goles_visitante > p.goles_local THEN 1 ELSE 0 END), 0) as pg_fuera,
            COALESCE(SUM(CASE WHEN p.id_visitante = e.id_equipo AND p.goles_local = p.goles_visitante THEN 1 ELSE 0 END), 0) as pe_fuera,
            COALESCE(SUM(CASE WHEN p.id_visitante = e.id_equipo AND p.goles_visitante < p.goles_local THEN 1 ELSE 0 END), 0) as pp_fuera,
            COALESCE(SUM(CASE WHEN p.id_visitante = e.id_equipo THEN p.goles_visitante ELSE 0 END), 0) as gf_fuera,
            COALESCE(SUM(CASE WHEN p.id_visitante = e.id_equipo THEN p.goles_local ELSE 0 END), 0) as gc_fuera

        FROM equipos e
        -- Hacemos un LEFT JOIN hacia los partidos reales activos que tengan goles asignados
        LEFT JOIN partidos p ON (e.id_equipo = p.id_local OR e.id_equipo = p.id_visitante) 
                            AND p.goles_local IS NOT NULL 
                            AND p.goles_visitante IS NOT NULL
        GROUP BY e.id_equipo, e.nombre
        -- Ordenamos estrictamente por puntos totales, diferencia de goles y goles marcados
        ORDER BY pt DESC, (gf - gc) DESC, gf ASC`;

    db.query(queryMaestraEnVivo, (err, rows) => {
        if (err) {
            console.error("Error en computo en vivo:", err);
